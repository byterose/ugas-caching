...
const getMiningRewards = (assetName, asset, assetPrice) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (assetName.toLowerCase().includes("ustonks")) {
            _tokenCount = Number(ethers_1.utils.formatUnits(getEmpInfo.collateralCount, 6));
        }
        else {
            _tokenCount = Number(ethers_1.utils.formatUnits(getEmpInfo.collateralCount, 18));
        }
        console.log("_tokenCount", _tokenCount.toString());
        // tokenPrice
        let _tokenPrice = tokenPrice;
        /// @dev In v2 we have to use the collateral price
        if (assetName.toLowerCase().includes("ustonks")) {
            _tokenPrice = 1;
        }
        else {
            _tokenPrice = yield getEthPrice();
        }
